    ...
    <section id="harga" className="py-24 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="opacity-50">Tidak ada biaya tersembunyi.</p>
        </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative p-6 rounded-2xl border transition-all duration-300 ${
                plan.popular
                  ? 'border-[#00d4ff]/50 bg-gradient-to-b from-[#00d4ff]/10 to-transparent shadow-[0_0_40px_rgba(0,212,255,0.1)]'
                  : 'border-black/5 dark:border-white/5 bg-white dark:bg-white/3'
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gradient-to-r from-[#00d4ff] to-[#0284c7] text-white text-xs font-bold">
                  Paling Populer
                </div>
              )}
              <div className="mb-6">
                ...
                <p className="text-sm opacity-50">{plan.desc}</p>
              </div>
              <ul className="space-y-3 mb-6">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm">
                    <Check size={14} className="text-[#00d4ff] flex-shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
